...
- [一、限流算法](#一限流算法)
  ...
  - [1.2 漏桶](#12-漏桶)
  - [1.3 令牌桶](#13-令牌桶)
- [二、Sentinel](#二sentinel)
  - [2.1 Sentinel的插槽时序](#21-sentinel的插槽时序)
  - [2.2 Sentinel的流控](#22-sentinel的流控)
    - [2.2.1 QPS限流](#221-qps限流)
    - [2.2.2 线程数限流](#222-线程数限流)
  - [2.3 Sentinel的降级](#23-sentinel的降级)
    - [2.3.1 rt（response time）降级](#231-rtresponse-time降级)
    - [2.3.2 异常降级](#232-异常降级)
- [三、Hystrix](#三hystrix)
  - [3.1 信号量隔离](#31-信号量隔离)
  - [3.2 线程池隔离](#32-线程池隔离)
...
- 6 `StatisticSlot#entry`，`StatisticSlot`是Sentinel的核心功能插槽之一，用于统计实时的调用数据。
  - 6.1. 新的请求到来，对node的`thread count`和`pass count`进行add操作。这个操作在`DegradeSlot#entry`执行结束并没有出现异常时执行，如果出现异常进行6.2的操作。
...
<!-- 
Hystrix使用了大量RxJava，代码扩展有一定门槛。

Hystrix如果流量是高扇出（如网关）的场景只适合使用信号量隔离。
Sentinel是Spring Cloud Alibaba微服务开发的一站式解决方案中服务限流降级的组件。

Hystrix以jar包的形式，使用轻便。
Sentinel要发挥最大作用的话，需要结合控制台、进行规则持久化操作、集群限流依赖token server。
Hystrix的信号量隔离与Sentinel的线程数限流类似，但是信号量隔离是微服务一级的，线程数限流是接口级别的。
 -->
